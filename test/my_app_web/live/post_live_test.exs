defmodule MyAppWeb.PostLiveTest do
  ...
  describe "Index" do
    ...
    test "saves new post", %{conn: conn} do
      ...
      assert_patch(index_live, ~p"/post/new")
      ...
      assert_patch(index_live, ~p"/post")
    ...
    test "updates post in listing", %{conn: conn, post: post} do
      ...
      assert_patch(index_live, ~p"/post/#{post}/edit")
  ...
  describe "Show" do
    ...
    test "displays post", %{conn: conn, post: post} do
      {:ok, _show_live, html} = live(conn, ~p"/post/#{post}")
    ...
    test "updates post within modal", %{conn: conn, post: post} do
      {:ok, show_live, _html} = live(conn, ~p"/post/#{post}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Post"

      assert_patch(show_live, ~p"/post/#{post}/show/edit")
      ...
      assert_patch(show_live, ~p"/post/#{post}")
